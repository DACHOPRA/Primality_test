...
int i, iPower, iNsmall=0, iPrime, iPrimeBPSW, iPrimeSBPSW, iPrimeMR2,
  ...
  iMaxBits;
unsigned long ulPix=0, ulUF, ulPSP_BPSW=0, ulPSP_SBPSW=0,
  ulPSP_MR2=0, ulPSP_LS=0, ulPSP_SLS=0, ulPSP_XSL=0, ulPSP_Fermat2=0;
double t0, lfSec, lf;
long double ldLB, ldUB, ldUF, ld1, ld2, ldMax;
...
iMaxBits=32*ceil(log(ldMax)/(32*log(2)));
...
while(1)
  ...
  if(mpz_fdiv_ui(mpzDelta, ulUF)==0)
    ...
    fprintf(stderr, "   Pix=%lu   T=%.3lf sec", ulPix, lfSec);
    vFlush();
    }
